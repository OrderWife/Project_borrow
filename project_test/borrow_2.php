...
document.form1.submit();
    ...
    $result_book = mysqli_query($conn, $sql_book);
                ...
                $sqlBookid2 = "SELECT bookid,statusbook FROM `bookborrow` where statusbook = 0 ORDER BY bookid DESC ";
                $result2 = mysqli_query($conn, $sql2);
                $resultBookid2 = mysqli_query($conn, $sqlBookid2);
             ?>
        <?php 
                $arrayBookid2 = array();
                $arrayBookid2[0] = 0;
                ...
                $rowArray = array();
                $conutOfRow = 0;
                while ($rowBookid2 = mysqli_fetch_array($resultBookid2, MYSQLI_NUM)) 
                  {
                    $arrayBookid2[$arKey]= $rowBookid2[0];
                    $arKey+=1;
                  }
                  $maxArr=sizeof($arrayBookid2);
                ...
                while ($row2 = mysqli_fetch_array($result2, MYSQLI_NUM)) 
                {
                  ...
                  {
                    if($row2[0] == $arrayBookid2[$arlength])
                    {
                        $check+=1;
                        continue;
                    }
                    ...
                    if($check == 0 && $arlength == ($maxArr -1))
                    {
                      ...
                      $rowArray[$conutOfRow][0] = $row2[0];
                      $rowArray[$conutOfRow][1] = $row2[1];
                      $conutOfRow+=1;
                      break;
                    }
           ...
           ?>  
            ...
            <?php 
             ...
             ?>
            <select style="width: 800px;" class="select2-single nodup" id="bookid1" name="bookid1" for='4'>
            <option value=" nodup"> -- เล่มโครงงาน --</option>

                   
           <?php 
                for($j = 0; $j < $conutOfRow; $j++)
                {
                  $tmp1= $rowArray[$j][0];
                  $tmp2= $rowArray[$j][1];
                  echo "<option value='$tmp1'>$tmp2</option>";
                }
             ?>
    ...
    include 'connect_book.php';
    //$bookid = $_GET['bookid'];
    $sql_book2 = "SELECT * FROM `book`

    INNER JOIN advisor ON book.advisorid=advisor.advisorid ";
    $result_book2 = mysqli_query($conn, $sql_book2);

        ?>
             
        
            เล่มที่ 2 : 
            <select style="width: 800px;" class="select2-single nodup" id="bookid2" name="bookid2" for='4'>
            <option value="bookid2"> -- เล่มโครงงาน --</option>
            <?php 
                for($j = 0; $j < $conutOfRow; $j++)
                {
                  $tmp1= $rowArray[$j][0];
    ...
    include 'connect_book.php';
    //$bookid = $_GET['bookid'];
    $sql_book3 = "SELECT * FROM `book`

    INNER JOIN advisor ON book.advisorid=advisor.advisorid ";
    $result_book3 = mysqli_query($conn, $sql_book3);
        ...
        ?>
            ...
            <?php 
                ...
                $sqlBookid3 = "SELECT bookid,statusbook FROM `bookborrow` where statusbook = 0 ORDER BY bookid DESC ";
                $result3 = mysqli_query($conn, $sql3);
                $resultBookid3 = mysqli_query($conn, $sqlBookid3);
             ?>
            <select style="width: 800px;" class="select2-single nodup" id="bookid3" name="bookid3" for='4' >
            <option value="bookid3"> -- เล่มโครงงาน --</option>
           ...
           ?>  
            ...
            #StartCheckMember_in_BlackList
                  ...
                  // }
                  while ($row = mysqli_fetch_array($result_member, MYSQLI_NUM)) {
                        $memberBl=false;
